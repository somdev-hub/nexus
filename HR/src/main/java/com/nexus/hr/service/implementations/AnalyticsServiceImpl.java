...
public class AnalyticsServiceImpl implements AnalyticsService {
	...
	private static HeroAnalyticsResponseDto.Values fillHeroAnalyticsResponseValues(Integer presentValue,
			Integer differenceValue, String comparisonWith) {
		...
		presentEmployeesCountValues.setTrend(differenceValue > 0 ? HeroAnalyticsResponseDto.Trend.INCREMENT
				: differenceValue < 0 ? HeroAnalyticsResponseDto.Trend.DECREMENT
						: HeroAnalyticsResponseDto.Trend.STABLE);
	...
	public ResponseEntity<?> getEmployeeAvgStrength(Long orgId) {
		...
		try {
			...
			List<Object[]> monthWiseData = timeManagementRepo.findMonthWiseEmployeePresenceLastYear(orgId, startYear,
					startMonthValue, endYear, endMonthValue);
		...
		try {
			...
			if (month == null || year == null) {
				return ResponseEntity.badRequest()
						.body("Invalid month-year format. Received: '" + monthYear
								+ "'. Expected format: 'APRIL 2026'");
			}

			System.out.println(
					"DEBUG: Fetching leave distribution for orgId=" + orgId + ", month=" + month + ", year=" + year);
	...
	public ResponseEntity<?> getPayrollYearly(Long orgId) {
		...
		try {
			...
			List<Object[]> payrollData = payrollRepo.getMonthlyAverageNetPayroll(orgId, startYear, startMonthValue,
					endYear, endMonthValue);
	...
	public ResponseEntity<?> getPayrollRoleWise(Long orgId, Map<String, List<Long>> roleEmpIdsMappings) {
		...
		try {
			...
			for (Map.Entry<String, List<Long>> entry : roleEmpIdsMappings.entrySet()) {
				...
				if (!empIds.isEmpty()) {
					List<Object[]> quarterlyData = payrollRepo.getQuarterlyAverageNetPayrollByEmployees(empIds,
							startYear, startMonthValue, endYear, endMonthValue);
	...
	public ResponseEntity<?> getLeavesDepartmentWise(Long orgId, String monthYear) {
		...
		try {
			...
			List<Object[]> departmentData = employeeLeavesRepo.getAverageLeavesByDepartment(orgId, startDate,
					endDateExclusive);
	...
	public ResponseEntity<?> getLeavesRoleWise(Long orgId, Map<String, List<Long>> roleEmpIdMap, String monthYear) {
		if (ObjectUtils.isEmpty(orgId) || ObjectUtils.isEmpty(roleEmpIdMap) || ObjectUtils.isEmpty(monthYear)) {
			return ResponseEntity.badRequest()
					.body("Organization ID, role-employee mappings and Month-Year are required");
		}
		try {
			Map<String, Integer> monthYearMap = parseMonthYear(monthYear);
			Integer month = monthYearMap.get("month");
			...
			for (Map.Entry<String, List<Long>> entry : roleEmpIdMap.entrySet()) {
				...
				if (empIds != null && !empIds.isEmpty()) {
					Double avgLeaves = employeeLeavesRepo.getAverageLeavesByEmployeeIds(orgId, empIds, startDate,
							endDateExclusive);
	...
	public ResponseEntity<?> getHeroAnalytics(Long orgId) {
		...
		try {
			...
			Integer presentEmployeesCount = hrEntityRepo.countPresentEmployees(orgId, date.getDayOfMonth(),
					date.getMonthValue(), date.getYear());
			Integer presentEmployeesCountDiffTodayVsYesterday = hrEntityRepo
					.getPresentEmployeesCountDiffTodayVsYesterday(orgId, date.getDayOfMonth(), date.getMonthValue(),
							date.getYear());
			HeroAnalyticsResponseDto.Values values = fillHeroAnalyticsResponseValues(presentEmployeesCount,
					presentEmployeesCountDiffTodayVsYesterday, "Yesterday");
			response.setPresentEmployees(values);

			Integer onLeaveEmployeesCount = employeeCount - presentEmployeesCount;
			Integer previousDayOnLeaveEmployeesCount = hrEntityRepo.getPreviousDayOnLeaveEmployeesCount(orgId,
					date.getDayOfMonth(), date.getMonthValue(), date.getYear());
			Integer onLeaveEmployeesCountDiffTodayVsYesterday = onLeaveEmployeesCount
					- previousDayOnLeaveEmployeesCount;
			HeroAnalyticsResponseDto.Values onLeaveEmployeesValues = fillHeroAnalyticsResponseValues(
					onLeaveEmployeesCount, onLeaveEmployeesCountDiffTodayVsYesterday, "Yesterday");
			...
			HeroAnalyticsResponseDto.Values hrRequestValues = fillHeroAnalyticsResponseValues(openHrRequests,
					hrRequestsCountDiffWeekly, "Previous Week");
	...
	/**
	 * Parse monthYear string in format "APRIL 2026" and return map with month and
	 * year
