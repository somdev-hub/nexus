...
public class HrServiceImpl implements HrService {
	...
	protected void processAsyncHrOperations(HrInitRequestDto hrInitRequestDto, HrEntity savedHrEntity,
		...
		try {
			...
			try {
				...
				commsService.sendCommunication(CommonConstants.CommsTriggerPoint.CANDIDATE_SELECTION_MAIL,
						emailAttachmentDtos, commsPayload);
	...
	public ResponseEntity<?> getPayrollEmployees(List<Long> empIds) {
		...
		try {
			...
			List<PayrollEmployeeResponse> payrollEmployees = empIds.stream().map(empId -> {
				...
				PayrollEmployeeResponse.PayrollEmployeeResponseBuilder responseBuilder = PayrollEmployeeResponse
						.builder()
						.employeeId(empId)
				...
				if (!payrollOptional.isEmpty()) {
					...
					log.debug("Found payroll record for employee {} with status: {}", empId,
	...
	@Override
	public ResponseEntity<?> getPayrollProcessed(Long orgId, Integer month, Integer year, Integer pageNo,
			Integer pageSize) {
		...
		try {
			log.info(
					"=== Fetching processed payroll records for orgId: {}, month: {}, year: {}, page: {}, size: {} ===",
			...
			log.info(
					"Successfully retrieved processed payroll records - Total elements: {}, Total pages: {}, Current page: {}",
	 ...
	 * 
	 * Approval levels:
	 * - AUTO: No human approval needed (auto-approved)
	 * - MANAGER: Requires manager or higher authority
	 * - DIRECTOR: Requires director authority
	 */
	@Override
	public ResponseEntity<?> checkApprovalAuthority(Long employeeId, String requiredLevel) {
		if (ObjectUtils.isEmpty(employeeId)) {
			throw new ServiceLevelException("HR Service", "Employee ID cannot be null or empty",
					"checkApprovalAuthority", "InvalidInput", "Employee ID is null or empty");
		}

		if (ObjectUtils.isEmpty(requiredLevel)) {
			throw new ServiceLevelException("HR Service", "Required level cannot be null or empty",
					"checkApprovalAuthority", "InvalidInput", "Required level is null or empty");
		}

		try {
			HrEntity hrEntity = hrEntityRepo.findByEmployeeId(employeeId)
					.orElseThrow(() -> new ResourceNotFoundException("HrEntity", "employeeId", employeeId));

			// Get current position
			Position currentPosition = hrEntity.getPositions().getLast();
			String positionTitle = currentPosition.getTitle().toUpperCase();

			// Determine authority based on position title
			boolean hasAuthority = false;

			switch (requiredLevel.toUpperCase()) {
				case "AUTO":
					// AUTO approval doesn't require human authority
					hasAuthority = true;
					break;
				case "MANAGER":
					// Manager level requires manager, senior manager, director, VP, or C-level
					hasAuthority = positionTitle.contains("MANAGER") ||
							positionTitle.contains("DIRECTOR") ||
							positionTitle.contains("VP") ||
							positionTitle.contains("VICE PRESIDENT") ||
							positionTitle.contains("CHIEF") ||
							positionTitle.contains("HEAD") ||
							positionTitle.contains("LEAD");
					break;
				case "DIRECTOR":
					// Director level requires director, VP, or C-level
					hasAuthority = positionTitle.contains("DIRECTOR") ||
							positionTitle.contains("VP") ||
							positionTitle.contains("VICE PRESIDENT") ||
							positionTitle.contains("CHIEF") ||
							positionTitle.contains("HEAD");
			...
			Map<String, Object> response = Map.of(
					...
					"currentPosition", currentPosition.getTitle(),
			...
			throw new ServiceLevelException("HR Service", "Exception occurred while checking approval authority",
					"checkApprovalAuthority", e.getClass().getName(), e.getMessage());
		}
	}
}
