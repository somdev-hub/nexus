...
public interface ApplicantRepo extends JpaRepository<Applicant, Long>, JpaSpecificationExecutor<Applicant> {
    ...
    @Query(value = "SELECT AVG(EXTRACT(DAY FROM (a.updated_on - a.applied_on))) FROM hr.t_hr_applicant_recruitment_mapping a JOIN hr.t_hr_recruitments r ON a.recruitment_recruitment_id = r.recruitment_id WHERE r.org_id = :orgId AND a.status = 'OFFER_ACCEPTED'", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(a) FROM hr.t_hr_applicant_recruitment_mapping a JOIN hr.t_hr_recruitments r ON a.recruitment_recruitment_id = r.recruitment_id WHERE r.org_id = :orgId AND a.status = 'OFFER_ACCEPTED'", nativeQuery = true)
    Long countOfferAccepted(@Param("orgId") Long orgId);

    // Count selected candidates
    @Query(value = "SELECT COUNT(a) FROM hr.t_hr_applicant_recruitment_mapping a JOIN hr.t_hr_recruitments r ON a.recruitment_recruitment_id = r.recruitment_id WHERE r.org_id = :orgId AND a.status = 'SELECTED'", nativeQuery = true)
    ...
    @Query(value = "SELECT COUNT(a) FROM hr.t_hr_applicant_recruitment_mapping a JOIN hr.t_hr_recruitments r ON a.recruitment_recruitment_id = r.recruitment_id WHERE r.org_id = :orgId AND EXTRACT(MONTH FROM a.applied_on) = EXTRACT(MONTH FROM CURRENT_DATE) AND EXTRACT(YEAR FROM a.applied_on) = EXTRACT(YEAR FROM CURRENT_DATE)", nativeQuery = true)
    Long countApplicationsThisMonth(@Param("orgId") Long orgId);

    // Count applications last month
    @Query(value = "SELECT COUNT(a) FROM hr.t_hr_applicant_recruitment_mapping a JOIN hr.t_hr_recruitments r ON a.recruitment_recruitment_id = r.recruitment_id WHERE r.org_id = :orgId AND EXTRACT(MONTH FROM a.applied_on) = EXTRACT(MONTH FROM (CURRENT_DATE - INTERVAL '1 month')) AND EXTRACT(YEAR FROM a.applied_on) = EXTRACT(YEAR FROM (CURRENT_DATE - INTERVAL '1 month'))", nativeQuery = true)
    Long countApplicationsLastMonth(@Param("orgId") Long orgId);

    // Count distinct orgs with open recruitments
    @Query("SELECT COUNT(DISTINCT r.orgId) FROM Recruitment r WHERE r.hiringStatus = 'OPEN' AND r.isActive = true")
    Long countDistinctOrgsWithOpenRecruitments();

    // Count all open recruitments
    @Query("SELECT COUNT(r) FROM Recruitment r WHERE r.hiringStatus = 'OPEN' AND r.isActive = true")
    Long countAllOpenRecruitments();

    // Count open recruitments current month
    @Query("SELECT COUNT(r) FROM Recruitment r WHERE r.orgId = :orgId AND r.hiringStatus = 'OPEN' AND r.isActive = true AND r.createdAt >= :monthStart")
    Long countOpenRecruitmentsCurrentMonth(@Param("orgId") Long orgId, @Param("monthStart") Timestamp monthStart);

    // Count by orgId, roleName, and hiringStatus
    @Query("SELECT COUNT(r) FROM Recruitment r WHERE r.orgId = :orgId AND r.hiringStatus = :hiringStatus AND r.roleName = :roleName")
    ...
    // Average time to hire overall (global, no org filter)
    @Query(value = "SELECT AVG(EXTRACT(DAY FROM (a.updated_on - a.applied_on))) FROM hr.t_hr_applicant_recruitment_mapping a WHERE a.status = 'OFFER_ACCEPTED'", nativeQuery = true)
    Double averageTimeToHireOverall();

    // @Query("SELECT a FROM Applicant a JOIN ApplicantRecruitmentMapping b ON
    // a.applicantId=b.applicant.applicantId WHERE b.recruitment.recruitmentId =
    // :recruitmentId AND (LOWER(a.applicantFirstName) LIKE LOWER(CONCAT('%', :name,
    // '%')) OR LOWER(a.applicantLastName) LIKE LOWER(CONCAT('%', :name, '%'))) AND
    // e.yearsOfExperience >= :yearsOfExperience")
    @Query("""
            SELECT DISTINCT a FROM Applicant a
                JOIN ApplicantRecruitmentMapping b ON a.applicantId = b.applicant.applicantId
                JOIN a.applicantExperiences e
    ...
    // @Query("SELECT a FROM Applicant a JOIN ApplicantRecruitmentMapping b ON
    // a.applicantId=b.applicant.applicantId WHERE b.recruitment.recruitmentId =
    // :recruitmentId AND a.applicantGender = :gender AND e.yearsOfExperience >=
    // :yearsOfExperience")
    @Query("""
            SELECT DISTINCT a FROM Applicant a
                JOIN ApplicantRecruitmentMapping b ON a.applicantId = b.applicant.applicantId
    ...
    @Query("SELECT COUNT(a) FROM Applicant a JOIN ApplicantRecruitmentMapping b ON a.applicantId=b.applicant.applicantId WHERE b.recruitment.orgId = :orgId AND b.appliedOn >= :weekStart")
    Long countApplicationsThisWeek(@Param("orgId") Long orgId, @Param("weekStart") java.sql.Timestamp weekStart);

    @Query("SELECT COUNT(arm) FROM ApplicantRecruitmentMapping arm WHERE arm.recruitment.recruitmentId = :recruitmentId")
    Long countByRecruitmentRecruitmentId(@Param("recruitmentId") Long recruitmentId);

    @Query("SELECT COUNT(arm) FROM ApplicantRecruitmentMapping arm WHERE arm.recruitment.orgId = :orgId AND arm.recruitment.roleName = :roleName")
    Long countApplicationsByRoleForOrg(@Param("orgId") Long orgId, @Param("roleName") String roleName);
}
