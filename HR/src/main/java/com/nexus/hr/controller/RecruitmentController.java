...
public class RecruitmentController {
    ...
    @GetMapping("/")
    public ResponseEntity<?> getAllRecruitments(@RequestParam(required = false) Boolean isActive, @RequestParam Long empId, @RequestParam(required = false) HiringType hiringType, @RequestParam(required = false) HiringStatus hiringStatus, @RequestParam(required = false, defaultValue ="0") Integer pageNo, @RequestParam(required = false, defaultValue = "10") Integer pageOffset) {
        return recruitmentService.getAllRecruitments(isActive, empId, hiringType, hiringStatus, PageRequest.of(pageNo, pageOffset));
    }
