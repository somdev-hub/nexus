...
public class ProductServiceImpl implements ProductService {
	...
	public ResponseEntity<?> getProductByIdAndOrg(Long id, Long orgId) {
		if (ObjectUtils.isEmpty(id) || ObjectUtils.isEmpty(orgId)) {
			return new ResponseEntity<ErrorResponse>(
					new ErrorResponse(
							"Product ID and Organization ID cannot be null or empty",
							HttpStatus.BAD_REQUEST.value(),
							Timestamp.valueOf(LocalDateTime.now()),
							"Invalid Product ID or Organization ID"),
					HttpStatus.BAD_REQUEST);
		}
		try {
			Product product = productRepo.findByIdAndOrg(id, orgId).orElse(null);
			if (ObjectUtils.isEmpty(product)) {
				return new ResponseEntity<ErrorResponse>(
						new ErrorResponse(
								...
								HttpStatus.NOT_FOUND.value(),
								Timestamp.valueOf(LocalDateTime.now()),
								"No product found with the given ID in the organization"),
						HttpStatus.NOT_FOUND);
			}
			return new ResponseEntity<>(modelMapper.map(product, ProductDto.class), HttpStatus.OK);
		} catch (Exception e) {
