...
public class OrganizationServiceImpl implements OrganizationService {
	...
	public ResponseEntity<?> getEmployeeDetails(Long userId) {
		...
		try {
			...
			if (hrResponse.getStatusCode().is2xxSuccessful() && hrResponse.getBody() != null) {
				...
				// hrResponse.getBody();
				Map<String, Object> combinedData = objectMapper.readValue(hrResponse.getBody().toString(),
						new TypeReference<>() {
						});
	...
	@Override
	public ResponseEntity<?> getEmployeesAttendance(Long orgId, Long deptId, String date, Integer pageNo,
			Integer pageOffset,
			String authHeader) {
		if (ObjectUtils.isEmpty(orgId)) {
		...
		try {
			...
			List<EmployeesAttendanceDto> attendanceResponses = attendanceDataList.stream()
					.map(attendanceData -> {
						...
						dto.setCheckInTime(
								attendanceData.get("checkInTime") != null ? attendanceData.get("checkInTime").toString()
										: "");
						dto.setCheckOutTime(attendanceData.get("checkOutTime") != null
								? attendanceData.get("checkOutTime").toString()
								: "");
	...
	@Override
	public ResponseEntity<?> getPayrollEmployees(Long orgId, Long deptId, String role, Integer pageNo,
			Integer pageOffset, String token) {
		if (ObjectUtils.isEmpty(orgId)) {
			throw new IllegalArgumentException("Organization ID cannot be null");
		}
		try {
			...
			ResponseEntity<?> response = restService.iamRestCall(builder.toUriString(), employeeIds, headers,
	...
	public ResponseEntity<?> getEmployeeThisMonthAttendance(Long id, String token) {
		...
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getEmployeeThisMonthAttendanceUrl());
			Map<String, String> headers = commonUtils.buildJsonHeaders(token);
	...
	@Override
	public ResponseEntity<?> getProcessedPayrolls(Long orgId, Integer month, Integer year, Integer pageNo,
			Integer pageOffset, String token) {
		if (ObjectUtils.isEmpty(orgId)) {
			throw new IllegalArgumentException("Organization ID cannot be null");
		}
		try {
			...
			map.forEach(data -> {
				...
				data.put("name", user.getName());
				data.put("department",
						user.getHeadedDepartments().stream().findFirst().map(Department::getDepartmentName)
								.orElse(user.getMemberOfDepartments().stream().findFirst()
	...
	public ResponseEntity<?> getPayrollInsights(Long orgId, String month, Integer year) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(webConstants.getPayrollInsightsUrl())
					...
					.queryParam("month", month)
					.queryParam("year", year);
			ResponseEntity<?> response = restService.iamRestCall(builder.toUriString(), null, null, HttpMethod.GET,
					null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
	...
	@Override
	public ResponseEntity<?> getManyHrRequests(Long orgId, String requestType, String status, Integer page,
		...
		try {
			...
			// Map<String, String> headers = commonUtils.buildJsonHeaders(token);
			return restService.iamRestCall(builder.toUriString(), null,
	...
	@Override
	public ResponseEntity<?> takeActionOnHrRequest(Long requestId, String action, String resolutionRemarks,
			String token) {
		...
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getTakeActionOnHrRequestUrl())
	...
	public ResponseEntity<?> getClosedHrRequests(Long orgId, Integer page, Integer offset, String token) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder.fromUriString(webConstants.getClosedHrRequestsUrl())
					...
					.queryParam("offset", offset);
			// Map<String, String> headers = commonUtils.buildJsonHeaders(token);
			response = restService.iamRestCall(builder.toUriString(), null,
					Map.of(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE), HttpMethod.GET, null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
	...
	public ResponseEntity<?> getTimeManagementQuickUpdate(Long empId) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getTimeManagementQuickUpdateUrl())
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get time management quick update: External API returned status: "
	...
	@Override
	public ResponseEntity<?> getTodayAppliedHrRequests(Long orgId, String status, Integer page, Integer offset,
			String empName) {
		if (ObjectUtils.isEmpty(orgId)) {
			throw new IllegalArgumentException("Organization ID is required");
		}
		...
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getTodayAppliedHrRequestsUrl())
					.queryParam("orgId", orgId)
					.queryParam("status", status)
					.queryParam("page", page)
					.queryParam("offset", offset);
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get today's applied HR requests: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> createEventOnboardingTemplate(String requestBody, String token) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getEventOnboardingTemplateUrl());
			Map<String, String> headers = commonUtils.buildJsonHeaders(token);
			response = restService.iamRestCall(builder.toUriString(), requestBody, headers, HttpMethod.POST, null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to create event onboarding template: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> updateEventOnboardingTemplate(String requestBody, Boolean templateUpdate, String token) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getEventOnboardingTemplateUrl())
					.queryParam("templateUpdate", templateUpdate);
			Map<String, String> headers = commonUtils.buildJsonHeaders(token);
			response = restService.iamRestCall(builder.toUriString(), requestBody, headers, HttpMethod.PUT, null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to update event onboarding template: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> addEventOnboardingTemplateParams(String requestBody, String token) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getEventOnboardingTemplateParamsUrl());
			Map<String, String> headers = commonUtils.buildJsonHeaders(token);
			response = restService.iamRestCall(builder.toUriString(), requestBody, headers, HttpMethod.POST, null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to add event onboarding template parameters: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> updateEventOnboardingTemplateParams(String requestBody, String token) {
		...
		try {
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to update event onboarding template parameters: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> getEventOnboardingTemplates(Long orgId) {
		...
		try {
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get event onboarding templates: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> getEventOnboardingTemplateDetails(Long eventTemplateId) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getEventOnboardingTemplateUrl());
			response = restService.iamRestCall(builder.toUriString() + "/" + eventTemplateId, null,
					Map.of(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE), HttpMethod.GET, null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get event onboarding template details: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> getEventOnboardingTemplateDetailsByName(Long orgId, String templateName) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getEventOnboardingTemplateUrl() + "/name")
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get event onboarding template details by name: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> triggerEventOnboardingMail(String payload) {
		...
		try {
			UriComponentsBuilder builder = UriComponentsBuilder
					.fromUriString(webConstants.getTriggerEventOnboardingMailUrl());
			response = restService.iamRestCall(builder.toUriString(), payload,
					Map.of(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE), HttpMethod.POST, null);
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to trigger event onboarding mail: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> getEventOnboardingHits(String templateName, Long orgId) {
		...
		try {
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get event onboarding hits: External API returned status: "
								+ response.getStatusCode(),
	...
	public ResponseEntity<?> getEventStatusBreakdown(String templateName, Long orgId) {
		...
		try {
			...
			if (!response.getStatusCode().is2xxSuccessful() || ObjectUtils.isEmpty(response.getBody())) {
				throw new ServiceLevelException(
						"OrganizationServiceImpl",
						"Failed to get event status breakdown: External API returned status: "
								+ response.getStatusCode(),
